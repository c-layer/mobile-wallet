...
// this.addPrivateAccount(<pwd>,
    '0x2b509a3f22fcf5890ce908d49ba20520c9cedc458180148b8fd5970606646513', 'Hot Account');
...
export class AccountProvider {
    ...
    public removeAccount(account: Account, password: string): Observable<Profile> {
        ...
        if (account == this.activeAccount) {
            this.setInMemoryActive(this.accounts[0]);
        }
    ...
    private setInMemoryActive(account: Account): void {
    ...
    public accountCanSend(account: Account): boolean {
    ...
    public accountCanSendSymbol(account: Account, network: string, symbol: string): boolean {
        return account.type == AccountType.HOT
            && this.getAccountPortfolio(account).filter(item => {
